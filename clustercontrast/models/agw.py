...
class embed_net_ori(nn.Module):
    ...
    def forward(self, x1, x2, modal=0,label_1=None,label_2=None):
        ...
        if self.training:
            label_rgb_feat = feat[:2*bat]
            label_ir_feat = feat[4 * bat:5 * bat]
            label_data = torch.cat((label_rgb_feat, label_ir_feat), dim=0)
            cls_score = self.classifier(label_data)
            label_d = torch.cat((x_pool[:2*bat],x_pool[4 * bat:5 * bat]),dim=0)
            return feat, feat[2*bat:single_size], feat[single_size + bat:], label_1, label_2, x_pool[2*bat:single_size], x_pool[single_size + bat:], cls_score,label_d # x_pool#, self.classifier(feat
...
def agw(pretrained=True,no_local= 'down', **kwargs):
