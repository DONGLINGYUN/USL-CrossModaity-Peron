...
from clustercontrast.models.resnet_agw import remove_fc
...
# from label_graph.loss.loss import euclidean_dist, CosineSimilarity
...
def get_train_loader_ir(args, dataset, height, width, batch_size, workers,
                        num_instances, iters, trainset=None, no_cam=False, train_transformer=None, model=None):
    ...
    if rmgs_flag:
        if no_cam:
            ...
            # sampler = GraphSampler(train_set, dataset.images_dir, train_transformer, model, batch_size=64,
            #                     num_instance=num_instances,
            #                     gal_batch_size=64, prob_batch_size=64, save_path=None, verbose=False, mode=1)
    else:
        sampler = None
...
def get_train_loader_color(args, dataset, height, width, batch_size, workers,
                           ...
                           train_transformer1=None, model=None):
    train_set = sorted(dataset.train) if trainset is None else sorted(trainset)
    rmgs_flag = num_instances > 0
    if rmgs_flag:
        if no_cam:
            ...
            # sampler = GraphSampler(train_set, dataset.images_dir, train_transformer, model, batch_size=64,
            #                     num_instance=num_instances,
            #                    gal_batch_size=64, prob_batch_size=64, save_path=None, verbose=False, mode=2)
    else:
        sampler = None
...
def remove_classifier(state_dict):
...
def create_model(args):
    ...
    # model = models.create('resnet_ibn50a', num_features=args.features, norm=True, dropout=args.dropout,
    #                       num_classes=0, pooling_type=args.pooling_type)
...
def main_worker_stage1(args, log_s1_name):
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            features_rgb, _ = extract_features(model, cluster_loader_rgb, print_freq=50, mode=2)
            ...
            features_ir, _ = extract_features(model, cluster_loader_ir, print_freq=50, mode=1)
        ...
        # ----------********loading 30% label_dataset_rgb*******-------------------
        rgblabel_train = dataset_rgb.train
        rgblabel_train.sort(key=takeSecond)
        truth_label = []
        len_rgb_label = int(len(rgblabel_train) * args.rate)
        dataset_label_rgb = rgblabel_train[:len_rgb_label]

        for i, (fname, label, cid) in enumerate(sorted(dataset_label_rgb)):
            if label != -1:
                truth_label.append(label)

        irabel_train = dataset_ir.train
        irabel_train.sort(key=takeSecond)
        len_ir_label = int(len(irabel_train) * args.rate)
        dataset_label_ir = irabel_train[:len_ir_label]

        irtruth_label = []
        for i, (fname, label, cid) in enumerate(sorted(dataset_label_ir)):
            if label != -1:
                irtruth_label.append(label)
        rgb_num_classes = len(set(truth_label))
        ir_num_classes = len(set(irtruth_label))
        assert rgb_num_classes == ir_num_classes
        ...
        train_transformer_rgb = T.Compose([
            ...
            ##
            # T.RandomApply([torchvision.transforms.ColorJitter(0.4, 0.4, 0.2, 0.1)]),
            # T.RandomApply([GaussianBlur(23)], p=0.1),
        ...
        train_loader_ir = get_train_loader_ir(args, dataset_ir, args.height, args.width,
                                              ...
                                              train_transformer=transform_thermal, model=model)
        ...
        train_loader_rgb = get_train_loader_color(args, dataset_rgb, args.height, args.width,
                                                  ...
                                                  train_transformer1=train_transformer_rgb1, model=model)

        # ---------------***add label***--------------------------
        train_labelloader_ir = get_train_loader_ir(args, dataset_ir, args.height, args.width,
                                                   args.batch_size, args.workers, args.num_instances, iters,
                                                   trainset=dataset_label_ir, no_cam=args.no_cam,
                                                   train_transformer=transform_thermal, model=model)
        train_labelloader_rgb = get_train_loader_color(args, dataset_rgb, args.height, args.width,
                                                       args.batch_size, args.workers, args.num_instances, iters,
                                                       trainset=dataset_label_rgb, no_cam=args.no_cam,
                                                       train_transformer=train_transformer_rgb,
                                                       train_transformer1=train_transformer_rgb1, model=model)
        ...
        train_labelloader_ir.new_epoch()
        train_labelloader_rgb.new_epoch()

        # trainer.train(epoch, train_loader_ir, train_loader_rgb, optimizer,
        #               print_freq=args.print_freq, train_iters=len(train_loader_ir))
        trainer.train(epoch, train_loader_ir, train_loader_rgb, train_labelloader_ir, train_labelloader_rgb, optimizer,
                      print_freq=args.print_freq, train_iters=len(train_loader_ir))
        ...
        if epoch >= 0 and ((epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
            ...
            query_img, query_label = process_test_regdb(data_path, trial=trial, modal='visible')  # thermal
            gall_img, gall_label = process_test_regdb(data_path, trial=trial, modal='thermal')  # visible
...
def main_worker_stage2(args, log_s1_name, log_s2_name):
    ...
    model.load_state_dict(remove_classifier(checkpoint['state_dict']), strict=False)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (fname, label, cid) in enumerate(sorted(dataset_label_rgb)):
            if label != -1:
                truth_label.append(label)
        irlabel_train = dataset_ir.train
        irlabel_train.sort(key=takeSecond)
        len_ir_label = int(len(irlabel_train) * args.rate)
        dataset_label_ir = irlabel_train[:len_ir_label]
        ...
        featureslabelir, _ = extract_features(model, cluster_loader_labelir, print_freq=50, mode=1)
        del cluster_loader_labelir
        featureslabelir = torch.cat([featureslabelir[f].unsqueeze(0) for f, _, _ in sorted(dataset_label_ir)], 0)
        rerank_dist_labelir = compute_jaccard_distance(featureslabelir, k1=args.k1, k2=args.k2,
                                                       search_option=3)  # rerank_dist_all_jacard[features_rgb.size(0):,features_rgb.size(0):]#
        pseudo_labelir = cluster_ir.fit_predict(rerank_dist_labelir)
        ir_score = silhouette_score(featureslabelir, pseudo_labelir)
        # print("伪标签ir轮廓系数:", ir_score)
        tir_score = silhouette_score(featureslabelir, irtruth_label)
        # print("真实标签ir轮廓系数:", tir_score)
        ir_rate = ir_score / tir_score
        # print("轮廓系数IR图像比值:", ir_rate)
        cluster_loader_labelrgb = get_test_loader(dataset_rgb, args.height, args.width,
                                                  256, args.workers,
                                                  testset=sorted(dataset_label_rgb))
        featureslabelrgb, _ = extract_features(model, cluster_loader_labelrgb, print_freq=50, mode=2)
        del cluster_loader_labelrgb
        featureslabelrgb = torch.cat([featureslabelrgb[f].unsqueeze(0) for f, _, _ in sorted(dataset_label_rgb)], 0)

        rerank_dist_labelrgb = compute_jaccard_distance(featureslabelrgb, k1=args.k1, k2=args.k2,
                                                        search_option=3)  # rerank_dist_all_jacard[features_rgb.size(0):,features_rgb.size(0):]#
        ...
        if epoch >= 0:
            ...
            # dist = cos(features_rgb, features_ir)
            # dis = torch.nn.CosineSimilarity()
            # dis = dis(features_rgb, features_ir)
            # dist_cm = euclidean_dist(features_rgb, features_ir).numpy()
            ###########################
            # test_dist = torch.einsum('ij,ji->ii', features_rgb, np.transpose(features_ir))
            ...
            for i in range(int(lenth * lenth_ratio)):
                ...
                #update_memory1 = trainer.memory_ir.features[int(trainer.memory_ir.features.size(0)/2)+key[1]]
                #trainer.memory_rgb.features[key[0]] = lamda_cm * trainer.memory_rgb.features[key[0]] + (
                #           1 - lamda_cm) * (update_memory)
                trainer.memory_rgb.features[key[0]] = trainer.memory_rgb.features[key[0]]

                #trainer.memory_rgb.features[int(trainer.memory_rgb.features.size(0)/2) + key[0]] = lamda_cm * trainer.memory_rgb.features[int(trainer.memory_rgb.features.size(0)/2) +key[0]] + (
                #        1 - lamda_cm) * (update_memory1)

                trainer.memory_ir.features[key[1]] = lamda_cm * trainer.memory_ir.features[key[1]] + (1 - lamda_cm) * (
                    update_memory)
                #trainer.memory_ir.features[key[1]] = trainer.memory_ir.features[key[1]]

                #trainer.memory_ir.features[int(trainer.memory_ir.features.size(0)/2) + key[1]] = lamda_cm * trainer.memory_ir.features[int(trainer.memory_ir.features.size(0)/2) + key[1]] + (1 - lamda_cm) * (
                #   update_memory1)
        ...
        train_labelloader_rgb.new_epoch()
        trainer.train(epoch, train_loader_ir, train_loader_rgb, train_labelloader_ir, train_labelloader_rgb, optimizer,
                      print_freq=args.print_freq, train_iters=len(train_loader_ir), i2r=i2r,
                      r2i=r2i)  # ,i2r=i2r, r2i=r2i
        # trainer.train(epoch, train_loader_ir, train_loader_rgb, optimizer,
        #               print_freq=args.print_freq, train_iters=len(train_loader_ir))
        ...
        if epoch >= 0 and ((epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1)):
            ...
            data_path = './data/regdb/'
            query_img, query_label = process_test_regdb(data_path, trial=trial, modal='visible')  # visible
            gall_img, gall_label = process_test_regdb(data_path, trial=trial, modal='thermal')  # thermal
...
if __name__ == '__main__':
    ...
    parser.add_argument('--memorybank', default='hybrid', choices=["hybrid", "normal"])
